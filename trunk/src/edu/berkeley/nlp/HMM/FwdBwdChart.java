...
//	 SCALING
		...
		// data
		protected double[][][] alphas; // alphas[t][q_t][q_substate] =

		// p(y_0,...,p_t,q_t)

		protected double[] alphasScale;

		protected double[][][] betas; // betas[t][q_t][q_substate] =
		...
		protected double[][][] gammas; // nodePosteriors[t][q_t][q_substate] =
		...
		public FwdBwdChart(SubphoneHMM hmm, double emissionAttenuation)
		{
//			this.emissionAttenuation = emissionAttenuation;
		...
		public void init(double[][] obs) {
			...
			if (T > myLength) {
				...
				betas = new double[T][hmm.numPhones][hmm.maxNumSubstates];
				gammas = new double[T][hmm.numPhones][hmm.maxNumSubstates];
				obsLikelihoods = new double[T][hmm.numPhones][hmm.maxNumSubstates];
...
//	 assumes that calc() has been called before
		public double getUnscaledLikelihood() {
			return alphas[T - 1][hmm.phoneIndexer.indexOf(Corpus.END_PHONE)][0];
		}

		
		public double getLogLikelihood() {
			double scaleFactor = alphasScale[T-1];
			return Math.log(alphas[T - 1][hmm.phoneIndexer.indexOf(Corpus.END_PHONE)][0]) + (100*scaleFactor);
		}
		...
		protected void calculateAlphas() {
			...
			// Basis
			alphas[0][hmm.phoneIndexer.indexOf(Corpus.START_PHONE)][0] = 1.0;
			alphasScale[0] = 0.0;
...
//								if (previousAlpha==0) continue;
						...
						if (t < T - 1 && alpha>0 && toPhone!=1){
							double obsLik = hmm.b[toPhone][toSubstate].evalPdf(observations[t]);
//							if (emissionAttenuation!=0) obsLik = Math.pow(obsLik, emissionAttenuation);
					...
					for (int fromSubstate = 0; fromSubstate < hmm.numSubstatesPerState[fromPhone]; ++fromSubstate) {
						...
						for (int toPhone : allowedPhonesAtTime(t+1)) {
							...
							for (int toSubstate = 0; toSubstate < hmm.numSubstatesPerState[toPhone]; ++toSubstate) {
								...
								double previousBeta = betatp1[toPhone][toSubstate];
								
								double obsProb = (t == T - 2) ? 1.0 : obsLikelihoods_t[toPhone][toSubstate];
//								assert obsProb > 0 && !Double.isInfinite(obsProb);
								beta += previousBeta *leaveStateProb
										* hmm.a[toSubstate][toPhone][fromPhone][fromSubstate] * obsProb;
						...
						betat[fromPhone][fromSubstate] = beta;
				...
				if (logScale != 0) {
					...
					for (int fromPhone : allowedPhonesAtTime(t)) {
						for (int fromSubstate = 0; fromSubstate < hmm.numSubstatesPerState[fromPhone]; ++fromSubstate) {
							betat[fromPhone][fromSubstate] /= scale;
						}
					}
				}
		...
		protected void calculateGammas() {
			...
			for (int t = 0; t < T-1; ++t) {
				for (int fromPhone : allowedPhonesAtTime(t)) {
					for (int fromSubstate = 0; fromSubstate < hmm.numSubstatesPerState[fromPhone]; ++fromSubstate) {
						double alpha_s = alphas[t][fromPhone][fromSubstate];
						double alpha_scale = alphasScale[t];
						double beta_s = betas[t][fromPhone][fromSubstate];
						...
						gammas[t][fromPhone][fromSubstate] = gamma;
					}
				}
			}
		}
		...
		public double getGamma(int phone, int substate, int t) {
			return gammas[t][phone][substate];
		}
		...
		public int getMostLikelySubstate(int t, int q_t){
			return DoubleArrays.argMax(gammas[t][q_t]);
		}
			...
			for (int t = T - 1; t >= 0; --t) {
				...
				for (int phone : allowedPhonesAtTime(t)) {
					...
					for (int substate = 0; substate < hmm.numSubstatesPerState[phone]; ++substate) {
							...
							double alpha_s = alphas[t][phone][substate];

							double alpha_scale = alphasScale[t];
							double beta_s = betas[t][phone][substate];
