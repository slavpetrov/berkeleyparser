...
public class GrammarTester implements Callable {
	...
	public static void main(String[] args) {
		...
		System.out.println("Loading trees from " + path
				+ " and using treebank type " + opts.treebank);
		...
		if (!(devTestSet || finalTestSet || trainTestSet)) {
			System.out
					.println("I didn't understand dev/final test set argument "
							+ testSetString);
			System.exit(1);
		}
		...
		if (opts.computeConstraints) {
			...
			String dirName = ".";
			String baseName = "tmp";
			String[] consArgsTrain = addOptions(args1, new String[] { "-logT",
					"" + opts.logT, "-maxL", "" + opts.maxSentenceLength,
					"-path", opts.path, "-filterStupidFrickinWHNP",
					opts.filterStupidFrickinWHNP ? "true" : "false",
					"-markUnaryParents", "true", "-out",
					dirName + "/" + baseName + "0_" + opts.section, "-in",
					opts.inFileName, "-section", opts.section, "-nChunks", "1",
					"-outputLog", dirName + "/" + baseName + ".cons.log" });

			ParserConstrainer.main(consArgsTrain);
			opts.cons = dirName + "/" + baseName + "0_" + opts.section
					+ "-0.data";
		}
		if (opts.cons != null)
		...
		String inFileName = (opts.testAll) ? opts.filePath + "/"
				+ opts.inFileName : opts.inFileName;
		if (inFileName == null) {
		...
		if (finalLevel != -1)
			System.out.println("Parsing with projected grammar from level "
					+ finalLevel + ".");
		boolean viterbiParse = opts.viterbi;
		if (viterbiParse)
			System.out
					.println("Computing viterbi derivation instead of max-rule parse.");
		// CoarseToFineMaxRuleParser parser = new
		// CoarseToFineTwoChartsParser(grammar, lexicon,
		// opts.unaryPenalty,finalLevel,viterbiParse,false,false,opts.accurate);
		...
		System.out
				.println("The computed F1,LP,LR scores are just a rough guide. They are typically 0.1-0.2 lower than the official EVALB scores.");
		...
		if (externalParserFactory != null) {
			// parser = externalParserFactory.newParser(grammar, lexicon,
			// spanPredictor);
			...
			if (opts.nGrammars != 1) {
				...
				parser = new CoarseToFineMaxRuleProductParser(grammars,
						lexicons, opts.unaryPenalty, -1, opts.viterbi, false,
						false, opts.accurate, false, true, true);
				...
				if (opts.smooth) {
					...
					// Smoother grSmoother = new
					// SmoothAcrossParentBits(0.01,grammar.splitTrees);
					// grammar.setSmoother(grSmoother);
					// grammar.smooth(false);

					// Smoother lexSmoother = new
					// SmoothAcrossParentBits(0.01,grammar.splitTrees);
				...
				if ("plain".equals(opts.parser)) {
					testTrees = Corpus.filterTreesForConditional(testTrees,
							opts.filterAllUnaries,
							opts.filterStupidFrickinWHNP, false);
					...
					if (grammar instanceof HierarchicalAdaptiveGrammar) {
						...
						parser = new ConstrainedHierarchicalTwoChartParser(
								grammar, lexicon, spanPredictor,
								grammar.finalLevel);
					} else
						parser = new ConstrainedTwoChartsParser(grammar,
								lexicon, spanPredictor);
					if (opts.viterbi)
						parser.viterbi = true;
				} else if ("basic".equals(opts.parser)) {
					parser = new ConstrainedArrayParser(grammar, lexicon,
		...
		if (opts.filterTrees)
			testTrees = Corpus.filterTreesForConditional(testTrees,
					opts.filterAllUnaries, opts.filterStupidFrickinWHNP, false);

		if (opts.nThreads > 1) {
			System.out.println("Parsing with " + opts.nThreads
					+ " threads in parallel.");

			MultiThreadedParserWrapper m_parser = new MultiThreadedParserWrapper(
					parser, opts.nThreads);
			int treeNumber = 0;
				...
				while (m_parser.hasNext()) {
					...
					if (kBestParsing) {
						...
						for (Tree<String> pTree : parsedTrees) {
							pTree = TreeAnnotations
									.unAnnotateTree(pTree, false);
		...
		if (!opts.testAll) {
			...
			for (Tree<String> testTree : testTrees) {
				...
				if (cons != null) {
					...
					if (!opts.doNOTprojectConstraints)
						parser.projectConstraints(cons[i],
								opts.allSubstatesAllowed);
					allowedStates = cons[i];
				}
				Tree<String> parsedTree = null;
				if (kBestParsing) {
					List<Tree<String>> list = parser.getKBestConstrainedParses(
							testSentence, posTags, opts.k);
					double bestFscore = 0;
					...
					parsedTree = TreeAnnotations.unAnnotateTree(parsedTree,
							false);
					if (useGoldPOS && parsedTree.getChildren().isEmpty()) { // parse
																			// error
																			// when
																			// using
																			// goldPOS,
				...
				if (opts.evaluateConstraints && cons != null) {
					numGoldPruned = countPrunedNodes(testTree, allowedStates,
							Numberer.getGlobalNumberer("tags"), false, 0,
							testTree.getYield().size());
					numPruned = countPrunedNodes(allowedStates,
							Numberer.getGlobalNumberer("tags"), false, 0,
							testTree.getYield().size());
					System.out.println("Pruned " + numGoldPruned
							+ " constituents.");
				...
				if (opts.printGoldTree)
					System.out
							.println("Gold: " + testTree.getChildren().get(0));

				eval.evaluate(parsedTree, testTree);
				if (++i > opts.maxSentences)
					break;
			}
			if (opts.evaluateConstraints)
				System.out.println("Pruned total of " + totalGoldPruned
						+ " gold constituents out of a total of " + totalPruned
						+ " constituents pruned.");
			eval.display(true);
			System.out
					.println("The computed F1,LP,LR scores are just a rough guide. They are typically 0.1-0.2 lower than the official EVALB scores.");
			...
			for (Tree<String> testTree : testTrees) {
				...
				if (cons != null) {
					...
					if (!opts.doNOTprojectConstraints)
						parser.projectConstraints(cons[k],
								opts.allSubstatesAllowed);
				}
				k++;
			}
			...
			for (int f = 0; f < fileList.length; f += nProcess) {
				...
				for (int i = 0; i < nProcess; i++) {
					String fName = (f + i < fileList.length) ? fileList[f + i]
							.getName() : fileList[f].getName();
					String thisGrammar = opts.filePath + "/" + fName;
					GrammarTester tester = new GrammarTester(thisGrammar,
							testTrees, maxSentenceLength, cons);
				...
				try {
					for (int i = 0; i < nProcess; i++) {
						Pair<EnglishPennTreebankParseEvaluator.LabeledConstituentEval<String>, String> res = (Pair<EnglishPennTreebankParseEvaluator.LabeledConstituentEval<String>, String>) submits[i]
								.get();
						System.out.print(res.getSecond() + "\t");
						double thisF1 = res.getFirst().display(true);
						if (opts.printAllF1)
							System.out.println(res.getSecond() + " had F1 "
									+ thisF1);
						if (thisF1 > bestF1) {
							bestF1 = thisF1;
			...
			finalGrammar.renameTo(new File(opts.filePath + "/"
					+ opts.inFileName));
			pool.shutdown();
		}
		if (!opts.testAll)
	...
	GrammarTester(String fName, List<Tree<String>> tT, int maxL,
			boolean[][][][][] c) {
		testTrees = tT;
		cons = c;
		fileName = fName;
	...
	public static List<Integer>[][][] loadData(String fileName) {
		...
		try {
			...
			data = (List<Integer>[][][]) in.readObject(); // Read the mix of
															// grammars
	...
	private static boolean isAllowed(String label, Numberer tagNumberer,
			boolean[][] cons, boolean isPreTerminal) {
		...
		for (int state = 0; state < cons.length; ++state) {
			...
			if (!isPreTerminal)
				unannotatedLabel = TreeAnnotations.unAnnotateTree(
						new Tree<String>(asString,
								Collections.singletonList(new Tree<String>(
										"FakeLabel"))), false).getLabel();
	...
	private static int countPrunedNodes(Tree<String> tree,
			boolean[][][][] cons, Numberer tagNumberer, boolean splitRoot,
			int from, int to) {

		int total = 0;
		if (!isAllowed(tree.getLabel(), tagNumberer, cons[from][to],
				tree.isPreTerminal())) {
			total += 1;
		}
		...
		// }
		// short nodeNumStates = allSplitTheSame ? numStates[0] :
		// numStates[label];
		...
		for (Tree<String> child : tree.getChildren()) {
			...
			total += countPrunedNodes(child, cons, tagNumberer, true, from,
					from + length);
			from += length;
	...
	public Pair<EnglishPennTreebankParseEvaluator.LabeledConstituentEval<String>, String> call()
			throws Exception {
		EnglishPennTreebankParseEvaluator.LabeledConstituentEval<String> eval = new EnglishPennTreebankParseEvaluator.LabeledConstituentEval<String>(
		...
		if (grammar instanceof HierarchicalAdaptiveGrammar) {
			lexicon.explicitlyComputeScores(grammar.finalLevel);
			parser = new ConstrainedHierarchicalTwoChartParser(grammar,
					lexicon, spanPredictor, grammar.finalLevel);
		} else
			parser = new ConstrainedTwoChartsParser(grammar, lexicon,
					spanPredictor);
		...
		return new Pair<EnglishPennTreebankParseEvaluator.LabeledConstituentEval<String>, String>(
				eval, fileName);
