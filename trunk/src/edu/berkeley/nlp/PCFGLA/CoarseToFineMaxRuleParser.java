 ...
 *         Viterbi scores, which involve a max. This is used by the Labelled
...
public class CoarseToFineMaxRuleParser extends ConstrainedArrayParser {
	...
	public double getLogLikelihood(Tree<String> tree) {
		...
		if (llParser == null)
			llParser = new ArrayParser(grammar, lexicon);
	...
	public double computeTightThresholds(List<String> sentence) {
		...
		for (int level = startLevel; level < endLevel; level++) {
			...
			if (level >= 0) {
				...
				System.out.println("Can set the threshold for level " + level
						+ " to " + minThresh);
				maxThresholds[level] = Math
						.min(maxThresholds[level], minThresh);
			}
			// pruneChart(minThresh-1, curGrammar.numSubStates, level);
			pruneChart(pruningThreshold[level + 1], curGrammar.numSubStates,
					level);
		}
		return -1.0;
	...
	private double getTightestThrehold(int start, int end, int state,
			boolean canStartWithUnary, int level) {
		boolean posLevel = (end - start == 1);
		if (posLevel)
			return -2;
	...
	public void doGoldInsideOutsideScores(Tree<StateSet> tree,
			List<String> sentence) {
		Grammar curGrammar = grammarCascade[endLevel - startLevel + 1];
		Lexicon curLexicon = lexiconCascade[endLevel - startLevel + 1];
		...
		double initVal = 0;
		int level = isBaseline ? 1 : endLevel;
		createArrays(false/* false */, curGrammar.numStates,
	...
	public double getSentenceProbability(int start, int end, boolean sumScores) {
		...
		if (sumScores) {
			for (int pState = 0; pState < numStates; pState++) {
				if (allowedStates[start][end + 1][pState] == false)
					continue;
