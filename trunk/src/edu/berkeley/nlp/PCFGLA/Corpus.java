...
public class Corpus {
	 ...
	 */
	private Corpus(String path, TreeBankType treebank, boolean onlyTest, int skipSection) {
		myTreebank = treebank;
...
//sentences.add("((S (NP (NP (DT No) (NN fiddling)) (PP (IN with) (NP (NNS systems) (CC and) (NNS procedures)))) (VP (MD will) (ADVP (RB ever)) (VP (VB prevent) (NP (NNS markets)) (PP (IN from) (S (VP (VBG suffering) (NP (NP (DT a) (NN panic) (NN wave)) (PP (IN of) (NP (NN selling))))))))) (. .)))");
      ...
      case 2:
        ...
        sentences.add("((S (Z1 (Z2 x) (NNPS x)) (U3 (Uu (A1 (NNP x1) (NNPS x2))))))");// (B (G x) (H x))))");
...
//      	sentences.add("((Y (C (K (B b)) (B b)) (Z (D d))))");
	 ...
	 * @throws Exception
	 */
	private void loadChinese(String pathChinese) throws Exception {
		System.out.print("Loading Chinese treebank trees...");
		boolean chineseSmall = false;
		if (!chineseSmall) {
...
//		trainTrees.addAll(readTrees(path,-1, Integer.MAX_VALUE,Charset.defaultCharset()));
	...
	private void loadCONLL(String path, boolean useLatinEncoding) throws Exception {
		...
		for (Tree t : trainTrees){
			if (t.getChildren().size()!=1) System.out.println("Malformed v: "+t);
		}
		for (Tree t : devTestTrees){
			if (t.getChildren().size()!=1) System.out.println("Malformed v: "+t);
		}
		for (Tree t : finalTestTrees){
			if (t.getChildren().size()!=1) System.out.println("Malformed t: "+t);
		}
		System.out.println("done");
	}
		...
		if (!onlyTest) {
			if (skipSection==-1)
				...
				if (skipSection==2) {
					...
					trainTrees.addAll(readTrees(pathWSJ, middle+100, 2199,Charset.defaultCharset())); // was 200, 2199
				}
	...
	private void loadGerman(String path) throws Exception {
		...
		for (Tree<String> tree : tmp){
			...
			else if (i>19601){ finalTestTrees.add(tree);}
...
//    			if (n.getLabel().equals("@WHNP^g") || (n.getLabel().equals("WHNP") && n.getChildren().size() > 1))
  ...
  return binarizeAndFilterTrees(trees, verticalAnnotations, horizontalAnnotations, sentenceMaxLength, binarization, manualAnnotation, VERBOSE,false);
  }
  
  public static List<Tree<String>> binarizeAndFilterTrees(
			List<Tree<String>> trees, int verticalAnnotations,
			int horizontalAnnotations, int sentenceMaxLength,
			Binarization binarization, boolean manualAnnotation, boolean VERBOSE, boolean markUnaryParents) {
