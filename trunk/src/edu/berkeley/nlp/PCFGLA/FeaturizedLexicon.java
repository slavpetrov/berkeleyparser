...
public class FeaturizedLexicon implements Lexicon, Serializable {
  ...
  private DifferentiableFunction objective(final double[][][] expectedCounts) { // tag, substate
    ...
    return new DifferentiableFunction() {
      ...
      public double[] derivativeAt(double[] x) {
        ...
        return DoubleArrays.add(gradient, regularizationGradient(x));
      }
  ...
  private void refeaturize() {
    ...
    for (int tag = 0; tag < numStates; tag++) {
      ...
      for (int globalWordIndex = 0; globalWordIndex < wordIndexer.size(); ++globalWordIndex) {
        ...
        List<String>[] features = featurizer.featurize(word, tag, numSubStates[tag], wordCounter[globalWordIndex], tagWordCounts[tag][globalWordIndex]);
  ...
  public double[] score(StateSet stateSet, short tag, boolean noSmoothing, boolean isSignature) {
    ...
    if (globalWordIndex < 0) { // not rare, so it can't be this tag.
      List<String>[] features = featurizer.featurize(stateSet.getWord(), tag, numSubStates[tag], 0, 0);
      for (int state = 0; state < numSubStates[tag]; ++state) {
        double score = 0.0;
  ...
  public void mergeStates(boolean[][][] mergeThesePairs, double[][] mergeWeights) {
  	...
  	for (int tag=0; tag<numStates; tag++){
  		this.expectedCounts[tag] = new double[newNumSubStates[tag]][wordIndexer.size()];
  	}
  }
  ...
  public FeaturizedLexicon projectLexicon(double[] condProbs, int[][] mapping, int[][] toSubstateMapping) {
    ...
    for (short tag = 0; tag < numStates; tag++) {
      newLexicon.expectedCounts[tag] = new double[newNumSubStates[tag]][wordIndexer.size()];
      for (int substate = 0; substate < newNumSubStates[tag]; substate++) {
        for (int word = 0; word < wordIndexer.size(); ++word) {
          newLexicon.expectedCounts[tag][substate][word] =
                  isLogarithmMode ? Math.exp(newScores[tag][substate][word])
  ...
  public FeaturizedLexicon copyLexicon() {
    ...
    for (int tag = 0; tag < numStates; tag++) {
      copy.expectedCounts[tag] = new double[numSubStates[tag]][wordIndexer.size()];
    }
