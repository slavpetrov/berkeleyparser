...
public class GrammarSmoother {
    ...
    double grammarSmoothing = Double.parseDouble(CommandLineUtils.getValueOrUseDefault(input, "-grsm","0.1"));
    
    String smootherStr = CommandLineUtils.getValueOrUseDefault(input, "-smooth", "NoSmoothing");
    Smoother smoother = null;
    if (smootherStr.equals("NoSmoothing")){
    	smoother = new NoSmoothing();
    	System.out.println("Using smoother no smoothing "+smootherStr);
    } else if (smootherStr.equals("SmoothAcrossParentSubstate")){
    	smoother = new SmoothAcrossParentSubstate(grammarSmoothing);
    	System.out.println("Using smoother substate "+smootherStr);
    }else if (smootherStr.equals("SmoothAcrossParentBits")) {
	//    	smoother = new SmoothAcrossParentBits(grammarSmoothing,gr.splitTrees);
    	System.out.println("Using smoother bits "+smootherStr);
    }
    else
    ...
    //gr.smooth();
		pData.gr=gr;
		LexiconInterface lex = pData.getLexicon();
