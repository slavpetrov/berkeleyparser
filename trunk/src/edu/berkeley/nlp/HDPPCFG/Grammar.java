...
public class Grammar implements java.io.Serializable {
  ...
  private List<UnaryRule>[] sumProductClosedUnaryRulesWithParent;
	 ...
	 */
	private short [][] closedViterbiPaths = null;
	private short [][] closedSumPaths=null;
	...
	/*
	 * public Iterator uRuleIteratorByParent(int state, int substate) { if (state >=
	 * unaryRulesWithParent.length) { return Collections.EMPTY_LIST.iterator(); }
	 * return unaryRulesWithParent[state][substate].iterator(); }
	 * 
	 * public Iterator uRuleIteratorByChild(int state, int substate) { if (state >=
	 * unaryRulesWithC.length) { return Collections.EMPTY_LIST.iterator(); }
	 * return unaryRulesWithC[state][substate].iterator(); }
	 * 
	 * 
	 * public Iterator bRuleIteratorByParent(int state, int substate) { if (state >=
	 * binaryRulesWithParent.length) { return Collections.EMPTY_LIST.iterator(); }
	 * return binaryRulesWithParent[state][substate].iterator(); }
	 * 
	 * public Iterator bRuleIteratorByRightChild(int state, int substate) { if
	 * (state >= binaryRulesWithRC.length) { return
	 * Collections.EMPTY_LIST.iterator(); } return
	 * binaryRulesWithRC[state][substate].iterator(); }
	 * 
	 * public Iterator bRuleIteratorByLeftChild(int state, int substate) { if
	 * (state >= binaryRulesWithLC.length) { return
	 * Collections.EMPTY_LIST.iterator(); } return
	 * binaryRulesWithLC[state][substate].iterator(); }
	 */
...
//			for (UnaryRule uRule : unaries){
	...
	public Pair<double[],int[][][]> getLinearizedGrammar() {
		int nRules = getNumberOfRules();
		return getLinearizedGrammar(nRules);
	}
	...
	public Pair<double[],int[][][]> getLinearizedGrammar(int nRules) {
		
		double[] probs = new double[nRules];
		int[][][] startIndex = new int[numStates][numStates][numStates];
		...
		for (int state = 0; state < numStates; state++) {
			...
			for (int i = 0; i < parentRules.length; i++) {
				BinaryRule bRule = parentRules[i];
				startIndex[state][bRule.leftChildState][bRule.rightChildState] = ind;
				double[][][] scores = bRule.getScores2();
				for (int j=0; j<scores.length; j++){
					for (int k=0; k<scores[j].length; k++){
						if (scores[j][k]!=null){	
							for (int l=0; l<scores[j][k].length; l++){
								...
								//if (val==Double.NEGATIVE_INFINITY) System.out.println("Base rule");//continue;//val = 10000;
								probs[ind++] = val;
							}
						}
					}
		  ...
		  UnaryRule uRule = unaries[r];
			...
			//for (UnaryRule uRule : unaries){
				if (uRule.childState==uRule.parentState) continue;
				startIndex[state][uRule.childState][0] = ind;
				double[][] scores = uRule.getScores2();
				for (int j=0; j<scores.length; j++){
					if (scores[j]!=null){	
						for (int k=0; k<scores[j].length; k++){
	...
	/*
	public double[] getLinearizedCounts(int nRules, int[][][] startIndex) {
		double[] probs = new double[nRules];
		int ind=0;
		for (int state = 0; state < numStates; state++) {
			BinaryRule[] parentRules = this.splitRulesWithP(state);
		...
		//for (BinaryRule bRule : binaryRuleCounter.keySet()){
				double[][][] bcounts = binaryRuleCounter.getCount(bRule);
				if (bcounts==null) continue;
				int thisStartIndex = startIndex[bRule.parentState][bRule.leftChildState][bRule.rightChildState];
				int curInd = 0;
				double[][][] scores = bRule.getScores2();
				for (int j=0; j<bcounts.length; j++){
					for (int k=0; k<bcounts[j].length; k++){
						if (scores[j][k]!=null && bcounts[j][k]!=null ){	
							for (int l=0; l<bcounts[j][k].length; l++){
								probs[thisStartIndex + curInd++] = bcounts[j][k][l];
							}
						}
					}
				}
		  ...
		  //UnaryRule uRule = unaries[r];
			//List<UnaryRule> unaries = this.getUnaryRulesByParent(state);
			//for (UnaryRule uRule : unaries){
			for (UnaryRule uRule : unaryRuleCounter.keySet()){
				if (uRule.childState==uRule.parentState) continue;
				double[][] scores = uRule.getScores2();
				double[][] ucounts = unaryRuleCounter.getCount(uRule);
				if (ucounts==null) continue; // -> this a unary chain and those are never directly observed
				int thisStartIndex = startIndex[uRule.parentState][uRule.childState][0];
				int curInd = 0;
				for (int j=0; j<ucounts.length; j++){
					if (ucounts[j]!=null ){	
						for (int k=0; k<ucounts[j].length; k++){
							probs[thisStartIndex + curInd++] = ucounts[j][k];
						}
					}
				}
			}
			...
			for (int i = 0; i < parentRules.length; i++) {
				...
				for (int j=0; j<scores.length; j++){
					for (int k=0; k<scores[j].length; k++){
						if (scores[j][k]!=null){	
							for (int l=0; l<scores[j][k].length; l++){
								double val = Math.exp(probs[ind++]);//probs[ind++];//Math.exp(probs[ind++]);
...
//									System.exit(1);
								...
								/*else{
									val = (val==10000) ? 0 : Math.exp(val);
									val = (val==Double.POSITIVE_INFINITY) ? 1 : val;
								}*/
								//val = (val < 0) ? 0 : val;
								scores[j][k][l] = val;
							}
		  ...
		  UnaryRule uRule = unaries[r];
				...
				for (int j=0; j<scores.length; j++){
					if (scores[j]!=null){	
						for (int k=0; k<scores[j].length; k++){
							...
							//if (val==0) continue; // -> probably a unary chain... need to take care of those...
							scores[j][k] = val;
						}
					}
				}
			...
			ind = rememberInd;*/
//			List<UnaryRule> unariesL = this.getUnaryRulesByParent(state);
//			for (UnaryRule uRule : unariesL){
			UnaryRule[] unaries = this.getClosedSumUnaryRulesByParent(state);
			for (int r = 0; r < unaries.length; r++) {
				UnaryRule uRule = unaries[r];
				if (uRule.childState==uRule.parentState) continue;
...
//								System.exit(1);
				...
				UnaryRule nonClosureRule = getUnaryRule(state, uRule.childState);
				if (nonClosureRule==null) continue;
				else nonClosureRule.setScores2(scores);
				unaryRuleMap.put(nonClosureRule,nonClosureRule);
			}
		}
	...
	public void init() {
		...
		//if (findClosedPaths) {
			closedViterbiPaths = new short[numStates][numStates];
		//}
		closedSumPaths = new short[numStates][numStates];
	 ...
	 * @param dummy
	 *          A dummy parameter because otherwise java objects that this method
	 *          shares the signature of Grammar(List of Tree of Strings).
	 */
	/*
	 * comment out unused constructor public Grammar(List<Tree<StateSet>>
	 * trainTrees, Grammar old_grammar) { this.tagNumberer =
	 * Numberer.getGlobalNumberer("tags"); unaryRuleCounter = new Counter<UnaryRule>();
	 * binaryRuleCounter = new Counter<BinaryRule>(); symbolCounter = new
	 * CounterMap<Integer,Integer>(); numStates = tagNumberer.total();
	 * numSubStates = old_grammar.numSubStates; init();
	 * 
	 * for (Tree<StateSet> trainTree : trainTrees) { tallyStateSetTree(trainTree,
	...
	public Grammar(short[] nSubStates, boolean findClosedPaths,
			Sparsifier sparsifier, Smoother smoother, Grammar oldGrammar, double thresh) {
		this.tagNumberer = Numberer.getGlobalNumberer("tags");
		this.findClosedPaths = findClosedPaths;
		this.sparsifier = sparsifier;
		...
		if (oldGrammar!=null) {
			...
			this.baselineGrammar = oldGrammar.baselineGrammar;
		} else {
  ...
  // If randomness > 0, then we hallucinate some counts and use them
  // to do the M-step
	public void optimize(double randomness, DiscreteDistribCollectionFactory ddcFactory, TopLevelDistrib[] topDistribs,
			UnaryDiscreteDistribCollection[] unaryBackboneDistribs, BinaryDiscreteDistribCollection[] binaryBackboneDistribs,
			DiscreteDistrib[][] unaryBinaryDistribution) {
		init();
//		checkNumberOfSubstates();
		...
		Random random = GrammarTrainer.RANDOM;
			...
			for (UnaryRule unaryRule : unaryRuleCounter.keySet()) {
				...
				for (int i = 0; i < unaryCounts.length; i++) { // child
					if (unaryCounts[i]==null)
						unaryCounts[i] = new double[numSubStates[unaryRule.getParentState()]];
					for (int j = 0; j < unaryCounts[i].length; j++) { // parent
						double r = random.nextDouble() * randomness + initBias(i, unaryCounts.length);
			...
			for (BinaryRule binaryRule : binaryRuleCounter.keySet()) {
				...
				for (int i = 0; i < binaryCounts.length; i++) { // left child
					for (int j = 0; j < binaryCounts[i].length; j++) { // right child
						if (binaryCounts[i][j]==null)
							binaryCounts[i][j] = new double[numSubStates[binaryRule.getParentState()]];
						for (int k = 0; k < binaryCounts[i][j].length; k++) { // parent
              double r = random.nextDouble() * randomness +
                initBias(i, binaryCounts.length) * initBias(j, binaryCounts[i].length);
							//double r = random.nextDouble() * randomness;
		...
		normalize(ddcFactory, topDistribs, unaryBackboneDistribs, binaryBackboneDistribs, unaryBinaryDistribution);
		smooth(); // this also adds the rules to the proper arrays
	}

  // Encourage previous substates to be used first
  // Larger is preferable
  // This doesn't really work
  public static double initBias(int i, int n) { return 0; }
  //public static double initBias(int i, int n) { return 0.1*(n-i)/n; }
			...
			for (int r=0; r<splitRulesWithP[state].length; r++){
				...
				for (int lC=0; lC<rule.scores.length; lC++){
					for (int rC=0; rC<rule.scores[lC].length; rC++){
						if (rule.scores[lC][rC]==null) continue;
						boolean isNull=true;
...
//								System.out.print(".");
						...
						if (isNull) rule.scores[lC][rC]=null;
					}
				}
			...
			for (UnaryRule rule : unaryRulesWithParent[state]){
				for (int c=0; c<rule.scores.length; c++){
					if (rule.scores[c]==null) continue;
					boolean isNull=true;
...
//		}
		...
		for (UnaryRule unaryRule : unaryRuleCounter.keySet()) {
//			double[][] unaryCounts = unaryRuleCounter.getCount(unaryRule);
			double[][] unaryCounts = smoother.smooth(unaryRule.getScores2(), unaryRule.parentState);
		...
		for (BinaryRule binaryRule : binaryRuleCounter.keySet()) {
//			double[][][] binaryCounts = binaryRuleCounter.getCount(binaryRule);
			double[][][] binaryCounts = smoother.smooth(binaryRule.getScores2(), binaryRule.parentState);
		...
		if (findClosedPaths) {
			...
			for (int i=0; i<numStates; i++) {
				for (int j=0; j<numStates; j++) {
					if (closedViterbiPaths[i][j]!=null)
						size++;
				}
			}
			System.out.println("Closed viterbi unary path data structure covers "
					+ size + " / " + maxSize + " = " + (((double) size) / maxSize) + " state pairs");
		}*/
		//checkNumberOfSubstates();
	 ...
	 */
	public void normalize(DiscreteDistribCollectionFactory ddcFactory, TopLevelDistrib[] topDistribs,
			UnaryDiscreteDistribCollection[] unaryBackboneDistribs, BinaryDiscreteDistribCollection[] binaryBackboneDistribs,
			DiscreteDistrib[][] unaryBinaryDistribution) {
		...
		if (useVarDP){
      logs("Grammar.normalize(): updating backbone");
			// update the backbone distributions first
			double[][][] countsOverUnaries = tallyUnaries();
			double[][][][] countsOverBinaries = tallyBinaries();
      //System.out.println("N " + numStates);
			for (int state=0; state<numStates; state++){
        //System.out.println("AFF " + state + " " + isGrammarTag[state]);
				if (!isGrammarTag[state]) continue;
				double[][] uCounts = countsOverUnaries[state];
        //System.out.println("FFF " + state + " " + uCounts);
				if (uCounts != null) unaryBackboneDistribs[state].update(uCounts);
				double[][][] bCounts = countsOverBinaries[state];
				if (bCounts != null) binaryBackboneDistribs[state].update(bCounts);
				for (int substate=0; substate < numSubStates[state]; substate++){
					...
					if (uCounts != null){
						for (int state1=0; state1 < numStates; state1++){
							unaryMass += uCounts[state1][substate];
						}
					}
					...
					if (bCounts != null){
						for (int state1=0; state1 < numStates; state1++){
							for (int state2=0; state2 < numStates; state2++){
								binaryMass += bCounts[state1][state2][substate];
							}
						}
					}
					
					unaryBinaryDistribution[state][substate].update(new double[]{unaryMass,binaryMass});
				}
			}
			
		}
		
    logs("Grammar.normalize(): updating unaries");
		for (UnaryRule unaryRule : unaryRuleCounter.keySet()) {
			double[][] unaryCounts = unaryRuleCounter.getCount(unaryRule);
			int parentState = unaryRule.getParentState();
			int childState = unaryRule.getChildState();
			int nParentSubStates = numSubStates[parentState];
			int nChildStates = numSubStates[childState];
      ...
      if(useVarDP) {
        ...
        if(unaryRule.params == null)
          unaryRule.params = ddcFactory.newUnarySubstate(
              topDistribs[childState], nChildStates, nParentSubStates);
        // Update parameters
        //System.out.println("UPDATE " + unaryRule.parentState + " -> " + childState);
        unaryRule.params.update(unaryCounts);
        // Get scores to parse with next round
        unaryCounts = unaryRule.params.getScores();
        double[][] backboneProb = unaryBackboneDistribs[parentState].getScores();
        for (int i = 0; i < nParentSubStates; i++) {
        	double backboneWeight = backboneProb[childState][i];
        	double unaryWeight = unaryBinaryDistribution[parentState][i].getScores()[0];
        	for (int state1=0; state1 < unaryCounts.length; state1++){
							if (unaryCounts[state1]!=null)
								unaryCounts[state1][i] *= backboneWeight * unaryWeight;
        	}
        }
      }
      else {
...
//      unaryRuleCounter.setCount(unaryRule,unaryCounts);
    ...
    logs("Grammar.normalize(): updating binaries");
		for (BinaryRule binaryRule : binaryRuleCounter.keySet()) {
			double[][][] binaryCounts = binaryRuleCounter.getCount(binaryRule);
      ...
      if(useVarDP) {
        // Create new parameters if they don't exist
        if(binaryRule.params == null)
          binaryRule.params = ddcFactory.newBinarySubstate(
              topDistribs[binaryRule.leftChildState],
              topDistribs[binaryRule.rightChildState],
              numSubStates[binaryRule.leftChildState],
              numSubStates[binaryRule.rightChildState],
              nParentSubStates);
        // Update parameters
        binaryRule.params.update(binaryCounts);
        // Get scores to parse with next round
        binaryCounts = binaryRule.params.getScores();
        double[][][] backboneProb = binaryBackboneDistribs[parentState].getScores();
        for (int i = 0; i < nParentSubStates; i++) {
        	double backboneWeight = backboneProb[binaryRule.leftChildState][binaryRule.rightChildState][i];
        	double binaryWeight = unaryBinaryDistribution[parentState][i].getScores()[1];
        	for (int state1=0; state1 < binaryCounts.length; state1++){
						for (int state2=0; state2 < binaryCounts[state1].length; state2++){
							if (binaryCounts[state1][state2]!=null)
								binaryCounts[state1][state2][i] *= backboneWeight * binaryWeight;
						}
        	}
        }
      }
...
//      binaryRuleCounter.setCount(binaryRule,binaryCounts);
	 ...
	 * @return
	 */
	private double[][][][] tallyBinaries() {
		// count up how often a state, substate produces a given child state
		// indexed by: parentState, lChildState, rChildState, parentSubstate
		double[][][][] counts = new double[numStates][][][];
		for (BinaryRule binaryRule : binaryRuleCounter.keySet()) {
			double[][][] binaryCounts = binaryRuleCounter.getCount(binaryRule);
			int parentState = binaryRule.parentState;
			int nParentSubStates = numSubStates[parentState];
        ...
        for (int k = 0; k < binaryCounts[j].length; k++) {
          if (binaryCounts[j][k]==null) continue;
          for (int i = 0; i < nParentSubStates; i++) {
             sum[i] += binaryCounts[j][k][i];
          }
        }
      }
			if (counts[parentState]==null) counts[parentState] = new double[numStates][numStates][nParentSubStates];
			ArrayUtil.addInPlace(counts[parentState][binaryRule.leftChildState][binaryRule.rightChildState],sum);
		}
		return counts;
	}
	 ...
	 * @return
	 */
	private double[][][] tallyUnaries() {
		// count up how often a state, substate produces a given child state
		// indexed by: state, childState, parentSubstate
		double[][][] counts = new double[numStates][][];
		for (UnaryRule unaryRule : unaryRuleCounter.keySet()) {
			double[][] unaryCounts = unaryRuleCounter.getCount(unaryRule);
			int parentState = unaryRule.getParentState();
			...
			for (int j = 0; j < unaryCounts.length; j++) {
				if (unaryCounts[j]==null) continue;
				for (int i = 0; i < nParentSubStates; i++) {
					sum[i] += unaryCounts[j][i];
				}
			}
			if (counts[parentState]==null) counts[parentState] = new double[numStates][nParentSubStates];
			ArrayUtil.addInPlace(counts[parentState][unaryRule.childState],sum);
		}
		return counts;
	}
...
//								System.out.println("Maybe an underflow: count "+val+" lambda "+lambda+" theta " +theta[j][i] + " div "+SloppyMath.lambert(val,(1+lambda)));
						}
					}
					unaryRuleCounter.setCount(unaryRule,theta);
				} 
			...
			for (BinaryRule binaryRule : binaryRuleCounter.keySet()) {
				...
				double[][][] theta = new double[binaryCounts.length][binaryCounts[0].length][];
				if (iter==1){
					// initialize the thetas 
					for (int j = 0; j < binaryCounts.length; j++) {
						for (int k = 0; k < binaryCounts[j].length; k++) {
						...
						for (int k = 0; k < binaryCounts[j].length; k++) {
							...
							for (int i = 0; i < nParentSubStates; i++) {
								...
								if (SloppyMath.isDangerous(theta[j][k][i])) 
									System.out.println("Maybe an underflow: count "+val+" lambda "+lambda+" theta " +theta[j][k][i] + " div "+SloppyMath.lambert(val,(1+lambda)));
   ...
   */
	...
	public void tallyStateSetTree(Tree<StateSet> tree, Grammar old_grammar, double dataFactor) {
		...
		tallyStateSetTree(tree, tree_score, tree_scale, old_grammar, dataFactor);
	}
	
	public void tallyStateSetTree(Tree<StateSet> tree, double tree_score, double tree_scale,
			Grammar old_grammar, double dataFactor) {
		if (tree.isLeaf())
			return;
		if (tree.isPreTerminal())
			return;
		...
		case 1:
			...
			for (short i = 0; i < nChildSubStates; i++) {
				...
				for (short j = 0; j < nParentSubStates; j++) {
					...
					ucounts[i][j] += logRuleCount * dataFactor;
				}
			}
			...
			for (short i = 0; i < nLeftChildSubStates; i++) {
				...
				for (short j = 0; j < nRightChildSubStates; j++) {
					...
					for (short k = 0; k < nParentSubStates; k++) {
						...
						bcounts[i][j][k] += logRuleCount * dataFactor;
					}
				}
			}
		...
		for (Tree<StateSet> child : children) {
			tallyStateSetTree(child, tree_score, tree_scale, old_grammar, dataFactor);
		}
	}
  ...
  // TMP: initialize for artificial grammar 7
  // Try to iniatialize with true grammar
	public void tallyUninitializedStateSetTreeG7(Tree<StateSet> tree) {
		if (tree.isLeaf())
			return;
		// the lexicon handles preterminal nodes
		if (tree.isPreTerminal())
		...
		case 1: // ROOT -> S
			...
			double[][] counts = new double[nChildSubStates][nParentSubStates];
			UnaryRule urule = new UnaryRule(parentState, childState, counts);
			counts = new double[nChildSubStates][nParentSubStates];
      counts[0][0] = 100;
			unaryRuleCounter.incrementCount(urule, counts);
			break;
		case 2: // S -> X X
			StateSet leftChild = children.get(0).getLabel();
			short lChildState = leftChild.getState();
			...
			double[][][] bcounts = new double[nLeftChildSubStates][nRightChildSubStates][nParentSubStates];
			BinaryRule brule = new BinaryRule(parentState, lChildState, rChildState, bcounts);
			bcounts = new double[nLeftChildSubStates][nRightChildSubStates][nParentSubStates];
      for(int i = 0; i < 4; i++)
        bcounts[i][i][0] = 100;
			binaryRuleCounter.incrementCount(brule, bcounts);
			break;
		default:
			throw new Error("Malformed tree: more than two children");
		}
		...
		for (Tree<StateSet> child : children) {
			tallyUninitializedStateSetTreeG7(child);
		}
	}
	
  // TMP: initialize for artificial grammar 8
  // For parent state 0 (S), initialize the binary rules to use the
  // first half of the children substates (of X)
  // For parent state 1 (T), use the other half (of X)
  // Test if without hierarchy, the binary rules will start using
  // the same children substates (of X)
	public void tallyUninitializedStateSetTreeG8(Tree<StateSet> tree) {
		if (tree.isLeaf())
			return;
		// the lexicon handles preterminal nodes
		if (tree.isPreTerminal())
      ...
      counts[0][0] = 100;
		...
		case 2: // S -> X X and T -> X X
			...
			BinaryRule brule = new BinaryRule(parentState, lChildState, rChildState, bcounts);
			bcounts = new double[nLeftChildSubStates][nRightChildSubStates][nParentSubStates];
      for(int i = 0; i < nLeftChildSubStates; i++) {
        for(int j = 0; j < nRightChildSubStates; j++) {
          if(parentState == 0) { // S uses these subcategories
            if(i < nLeftChildSubStates/2 && j < nRightChildSubStates/2)
              bcounts[i][j][0] = 100;
          }
          else { // T uses these subcategories
            if(i >= nLeftChildSubStates/2 && j >= nRightChildSubStates/2)
              bcounts[i][j][0] = 100;
          }
        }
		...
		for (Tree<StateSet> child : children) {
			tallyUninitializedStateSetTreeG8(child);
		}
	}
	...
	*/
	...
	public List<short[]> getBestViterbiPath(short pState, short np, short cState, short cp) {
		...
		if (!findClosedPaths) {
			...
			while (state[0]!=cState || state[1]!=cp) {
				path.add(state);
				state[0] = closedViterbiPaths[state[0]][state[1]];
			}
	...
	public short getUnaryIntermediate(short start, short end){
					...
					if (state==childState){
						...
						for (int cp = 0; cp < nChildSubStates; cp++) {
							...
							for (int np = 0; np < nParentSubStates; np++) {
											// sum over intermediate substates
								double val = scores[cp][np];
								scoresSum[cp][np] += val;
								total += val;
								if (val > scoresMax[cp][np]) {
									scoresMax[cp][np] = val;
									bestMaxIntermed = -1;
								}
							}
		...
		if (findClosedPaths) {
			for (short i = 0; i < rule.scores.length; i++) {
				for (short j = 0; j < rule.scores[i].length; j++) {
					...
					closedViterbiPaths[rule.parentState][rule.childState][i][j] = pair; */
				}
			}
		}
	}
...
*/	}
	...
	public UnaryRule getUnaryRule(short pState, short cState) {
		uSearchRule.setNodes(pState, cState);
		UnaryRule r = unaryRuleMap.get(uSearchRule);
		return r;
	}
	...
	public BinaryRule getBinaryRule(short pState, short lState, short rState) {
		bSearchRule.setNodes(pState, lState, rState);
		BinaryRule r = binaryRuleMap.get(bSearchRule);
		return r;
	}
	...
	// currently not used
	/**
	 * Create a duplicate grammar, but with one substate split. Some randomness is
	 * added to break the symmetry.
	 * 
	 * @param state
	 * @param oldsubstate
	 * @param newsubstate1
	 ...
	 * @return
	 */
	// public Grammar splitState(int state, int oldsubstate, int newsubstate1, int
	// newsubstate2, double randomness) {
	// int[] numSubStates = this.numSubStates.clone();
	// numSubStates[state]+=1;
	// /** a new grammar to fill */
	// Grammar grammar = new Grammar(numSubStates);
	// Random random = new Random();
	// for (Rule r : allRules) {
	// if (r instanceof BinaryRule) {
	// BinaryRule oldRule = (BinaryRule) r;
	// BinaryRule newRule = null;
	// ArrayList<BinaryRule> parentSplitRules = new ArrayList<BinaryRule>();
	// ArrayList<BinaryRule> lcSplitRules = new ArrayList<BinaryRule>();
	// ArrayList<BinaryRule> rcSplitRules = new ArrayList<BinaryRule>();
	// //split all rules based on head; add no randomness
	// if (oldRule.getParentState()==state &&
	// oldRule.getParentSubState()==oldsubstate) {
	// BinaryRule b = oldRule;
	// double newScore = b.score()/2;
	// //add the first variant
	// newRule = new BinaryRule(b);
	// newRule.setScore(newScore);
	// newRule.parentSubState = newsubstate1;
	// parentSplitRules.add(newRule);
	// //add the second variant
	// newRule = new BinaryRule(b);
	// newRule.setScore(newScore);
	// newRule.parentSubState = newsubstate2;
	// parentSplitRules.add(newRule);
	// } else {
	// parentSplitRules.add(oldRule);
	// }
	// //split all rules based on left
	// for (BinaryRule b : parentSplitRules) {
	// if (b.getLeftChildState()==state && b.getLeftChildSubState()==oldsubstate)
	// {
	// double newScore = b.score()/2;
	// double scoreDiff = newScore * randomness / 100 * (random.nextDouble()-0.5);
	// //add the first variant
	// newRule = new BinaryRule(b);
	// newRule.setScore(newRule.score()/2 + scoreDiff);
	// newRule.leftChildSubState = newsubstate1;
	// lcSplitRules.add(newRule);
	// //add the second variant
	// newRule = new BinaryRule(b);
	// newRule.setScore(newRule.score()/2 - scoreDiff);
	// newRule.leftChildSubState = newsubstate2;
	// lcSplitRules.add(newRule);
	// } else {
	// lcSplitRules.add(b);
	// }
	// }
	// //split all rules based on right
	// for (BinaryRule b : lcSplitRules) {
	// if (b.getRightChildState()==state &&
	// b.getRightChildSubState()==oldsubstate) {
	// double newScore = b.score()/2;
	// double scoreDiff = newScore * randomness / 100 * (random.nextDouble()-0.5);
	// //add the first variant
	// newRule = new BinaryRule(b);
	// newRule.setScore(newRule.score()/2 + scoreDiff);
	// newRule.rightChildSubState = newsubstate1;
	// rcSplitRules.add(newRule);
	// //add the second variant
	// newRule = new BinaryRule(b);
	// newRule.setScore(newRule.score()/2 - scoreDiff);
	// newRule.rightChildSubState = newsubstate2;
	// rcSplitRules.add(newRule);
	// } else {
	// rcSplitRules.add(b);
	// }
	// }
	// //add all the split-up rules into the new grammar
	// for (BinaryRule b : rcSplitRules) {
	// grammar.addBinary(b);
	// }
	// }
	// else if (r instanceof UnaryRule) {
	// UnaryRule oldRule = (UnaryRule) r;
	// UnaryRule newRule = null;
	// ArrayList<UnaryRule> parentSplitRules = new ArrayList<UnaryRule>();
	// ArrayList<UnaryRule> childSplitRules = new ArrayList<UnaryRule>();
	// //split all rules based on head
	// if (oldRule.getParentState()==state &&
	// oldRule.getParentSubState()==oldsubstate) {
	// UnaryRule u = oldRule;
	// double newScore = u.score()/2;
	// //add the first variant
	// newRule = new UnaryRule(u);
	// newRule.setScore(newScore);
	// newRule.parentSubState = newsubstate1;
	// parentSplitRules.add(newRule);
	// //add the second variant
	...
	// parentSplitRules.add(oldRule);
	// }
	// //split all rules based on child
	// for (UnaryRule u : parentSplitRules) {
	// if (u.getChildState()==state && u.getChildSubState()==oldsubstate) {
	// double newScore = u.score()/2;
	// double scoreDiff = newScore * randomness / 100 * (random.nextDouble()-0.5);
	// //add the first variant
	// newRule = new UnaryRule(u);
	// newRule.setScore(newRule.score()/2 + scoreDiff);
	// newRule.childSubState = newsubstate1;
	// childSplitRules.add(newRule);
	// //add the second variant
	// newRule = new UnaryRule(u);
	// newRule.setScore(newRule.score()/2 - scoreDiff);
	// newRule.childSubState = newsubstate2;
	// childSplitRules.add(newRule);
	// } else {
	// childSplitRules.add(u);
	// }
	// }
	// //add all the split-up rules into the new grammar
	// for (UnaryRule u : childSplitRules) {
	// grammar.addUnary(u);
	// }
	// }
	// }
	 ...
	 * @return
	 */
	public Grammar splitAllStates(double randomness, int[] counts, boolean moreSubstatesThanCounts) {
		if (logarithmMode) {
		...
		for (short i = 0; i < numSubStates.length; i++) {
			...
			// THIS CAN MESS UP THE SPLIT TREES AND HAS THEREFORE BEEN TEMPORARILY DISABLED
...
//			}
		...
		Grammar grammar = new Grammar(newNumSubStates, findClosedPaths, sparsifier, smoother, this, threshold);
		Random random = GrammarTrainer.RANDOM;
		//for (Rule r : allRules) {
		//	if (r instanceof BinaryRule) {
		//BinaryRule oldRule = (BinaryRule) r;
		for (BinaryRule oldRule : binaryRuleMap.keySet()) {
			...
			int parentSplitFactor = oldRule.getParentState() == 0 ? 1 : 2; // should
			// never
			// happen, but
			// to be on
			// the safe
			// side
			if (newNumSubStates[oldRule.parentState]==numSubStates[oldRule.parentState]){parentSplitFactor=1;}
			int lChildSplitFactor = 2;
			if (newNumSubStates[oldRule.leftChildState]==numSubStates[oldRule.leftChildState]){lChildSplitFactor=1;}
			int rChildSplitFactor = 2;
			if (newNumSubStates[oldRule.rightChildState]==numSubStates[oldRule.rightChildState]){rChildSplitFactor=1;}
				...
				for (short rcS = 0; rcS < oldScores[0].length; rcS++) {
					...
					for (short lc = 0; lc < lChildSplitFactor; lc++) {
						for (short rc = 0; rc < rChildSplitFactor; rc++) {
							...
							newScores[newLCS][newRCS] = new double[newNumSubStates[oldRule.parentState]];
						}
					}
							...
							for (short lc = 0; lc < lChildSplitFactor; lc++) {
								...
								* (random.nextDouble() - 0.5);
								...
								for (short rc = 0; rc < rChildSplitFactor; rc++) {
									...
									short newLCS = (short)(lChildSplitFactor * lcS + lc);
									short newRCS = (short)(rChildSplitFactor * rcS + rc);
									newScores[newLCS][newRCS][newPS] = sparsifier
											.splitBinaryWeight(oldRule.getParentState(), pS,
													oldRule.getLeftChildState(), lcS, oldRule
															.getRightChildState(), rcS, newPS, newLCS,
													newRCS, lChildSplitFactor, rChildSplitFactor,
													randomComponentLC, randomComponentRC, score, tagNumberer);
								}
							}
						}
					}
...
//				UnaryRule oldRule = (UnaryRule) r;
		for (UnaryRule oldRule : unaryRuleMap.keySet()){
			...
			short parentSplitFactor = oldRule.getParentState() == 0 ? (short)1 : (short)2;
			if (newNumSubStates[oldRule.parentState]==numSubStates[oldRule.parentState]){parentSplitFactor=1;}
			int childSplitFactor = 2;
			if (newNumSubStates[oldRule.childState]==numSubStates[oldRule.childState]){childSplitFactor=1;}
			double[][] oldScores = oldRule.getScores2();
			double[][] newScores = new double[newNumSubStates[oldRule.childState]][];
			...
			for (short cS = 0; cS < oldScores.length; cS++) {
				...
				for (short  c = 0; c < childSplitFactor; c++) {
					...
					newScores[newCS]= new double[newNumSubStates[oldRule.parentState]];
				}
				...
				for (short pS = 0; pS < oldScores[cS].length; pS++) {
					...
					for (short p = 0; p < parentSplitFactor; p++) {
						...
						double randomComponent = score / divFactor * randomness / 100
						* (random.nextDouble() - 0.5);
						// split on child
						for (short  c = 0; c < childSplitFactor; c++) {
							if (c == 1) {
							...
							short newPS = (short)(parentSplitFactor * pS + p);
							short newCS = (short)(childSplitFactor * cS + c);
							newScores[newCS][newPS] = sparsifier.splitUnaryWeight(
									oldRule.getParentState(), cS, oldRule.getChildState(), pS,
									newPS, newCS, childSplitFactor, randomComponent, score, tagNumberer);
						}
					}
				}
			}
						...
						if (partners[cS][j][0]==j) {
							...
							if (parentSplit==2) {
								for (int js = 0; js < childSplit; js++) {
									...
									if (mergeWeightSum==0)
										mergeWeightSum = 1;
									scores[js][0] = scores[js][1] = ((scores[js][0] * 
											mergeWeights[pS][partners[pS][i][0]]) + (scores[js][1] * 
													mergeWeights[pS][partners[pS][i][1]])) / mergeWeightSum;
								}
							}
							for (int is = 0; is < parentSplit; is++) {
								for (int js = 0; js < childSplit; js++) {
 ...
 * @param mergeThesePairs
	...
	public boolean isGrammarTag(int n){
