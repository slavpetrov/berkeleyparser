...
public class ConstrainedArrayParser extends ArrayParser implements ConstrainedParser{
...
//the chart is now using scaled probabilities, NOT log-probs.
  ...
  public ConstrainedArrayParser(Grammar gr, LexiconInterface lex, short[] nSub) {
		super(gr, lex);
            ...
            boolean hasRightPOSChild = !grammar.isGrammarTag(rState);
						...
						for (int np = 0; np < nParentSubStates; np++) {
							...
							for (int split = min; split <= max; split++) {
								double totalScoreThisSplit = Double.NEGATIVE_INFINITY;
								...
								for (int lp = 0; lp < scores.length; lp++) {
									...
									for (int rp = 0; rp < scores[0].length; rp++) {
										...
										if (hasLeftPOSChild||hasRightPOSChild) totalScoreThisSplit = SloppyMath.logAdd(totalScoreThisSplit,tot);
									}
								}
								if (totalScoreThisSplit > bestIScore) bestIScore = totalScoreThisSplit;
							}
				...
				for (int pState : possibleSt){//int pState=0; pState<0; pState++){//
					...
					for (int r = 0; r < unaries.length; r++) {
						...
						boolean hasPOSChild = !grammar.isGrammarTag(cState);
						...
						for (int np = 0; np < nParentSubStates; np++) {
							...
							for (int cp = 0; cp < scores.length; cp++) {
								...
								if (tot >= bestIScore) { bestIScore = tot; }
								if (hasPOSChild) totalScore = SloppyMath.logAdd(totalScore,tot);
							}
							if (totalScore > bestIScore) bestIScore = totalScore;
...
//							&& lexicon.getAllTags().contains(cState)) {
	...
	for (int split = start + 1; split < end; split++) {
      ...
      boolean hasRightPOSChild = !grammar.isGrammarTag(rState);
			...
			double[][][] scores = br.getScores2();
			double sumBest = Double.NEGATIVE_INFINITY;
			double indBest = Double.NEGATIVE_INFINITY;
			int bestLP = -1, bestRP = -1;
			for (int lp=0; lp<scores.length; lp++){
				for (int rp=0; rp<scores[lp].length; rp++){
					if (scores[lp][rp]==null) continue;
					double score = scores[lp][rp][gp] + iScore[start][split][lState][lp]
								+ iScore[split][end][rState][rp];
					if (hasLeftPOSChild||hasRightPOSChild) sumBest = SloppyMath.logAdd(sumBest,score);
					if (indBest < score) {
							indBest = score;
							bestLP = lp;
							bestRP = rp;
			...
			sumBest = Math.max(indBest,sumBest);
			if (matches(sumBest, bestScore)) {
				// build binary split
				Tree<String> leftChildTree = extractBestParse(lState, bestLP, start, split, sentence);
				Tree<String> rightChildTree = extractBestParse(rState, bestRP, split, end, sentence);
	...
	for (int r = 0; r < unaries.length; r++) {
		...
		for (int cp=0; cp<scores.length; cp++){
			...
			if (hasPOSChild) sumBest = SloppyMath.logAdd(sumBest,score);
			if (indBest < score){
				indBest = score;
				bestCP = cp;
			}
		...
		sumBest = Math.max(indBest, sumBest);
		if ((cState != ur.parentState || bestCP != gp) && matches(sumBest, bestScore)) {
			// build unary
			Tree<String> childTree = extractBestParse(cState, bestCP, start, end, sentence);
			List<Tree<String>> children = new ArrayList<Tree<String>>();
...
//	short bestBLp, bestBRp;
	...
	for (int split = start + 1; split < end; split++) {
      ...
      boolean hasRightPOSChild = !grammar.isGrammarTag(rState);
			...
			double bestB = Math.max(indBest,sumBest);
			bestBScore = Math.max(bestBScore,bestB);
			if (matches(bestB, bestScore)) {
				// build binary split
				Tree<StateSet> leftChildTree = extractBestStateSetTree(lState, bestLP, start, (short)split, sentence);
				Tree<StateSet> rightChildTree = extractBestStateSetTree(rState, bestRP, (short)split, end, sentence);
	...
	// check unaries
	double bestU = Double.NEGATIVE_INFINITY, bestUScore = Double.NEGATIVE_INFINITY;;
	UnaryRule[] unaries = grammar.getClosedViterbiUnaryRulesByParent(gState);
	for (short r = 0; r < unaries.length; r++) {
		...
		bestU = Math.max(indBest,sumBest);
		bestUScore = Math.max(bestUScore,bestU); 
		if ((cState != ur.parentState || bestCP != gp) && matches(bestU, bestScore)) {
			// build unary
			Tree<StateSet> childTree = extractBestStateSetTree(cState, bestCP, start, end, sentence);
			List<Tree<StateSet>> children = new ArrayList<Tree<StateSet>>();
			children.add(childTree);
			StateSet root = new StateSet(gState,one);
			root.allocate();
