 ...
 *
 */
public class ConstrainedCoarseToFineTwoChartsParser extends ConstrainedCoarseToFineParser {
	 ...
	 * @param score
	 */
	public ConstrainedCoarseToFineTwoChartsParser(Grammar gr, Lexicon lex, double unaryPenalty, int endL, boolean viterbi, boolean sub, boolean score) {
...
/*				if (start==1){
	...
	protected void createArrays(boolean firstTime, int numStates, short[] numSubStatesArray, int level, double initVal, boolean justInit) {
	  ...
	  public void doPreParses(List<String> sentence, Tree<StateSet> tree,boolean noSmoothing){
	  	...
	  	for (int level=1; level< endLevel; level++){ // was <=
		    ...
		    initializeChart(sentence,curLexicon,level<0,noSmoothing);
          ...
          if (diff > 1) {
            ...
            int nParentStates = numSubStatesArray[pState]; // == scores[0].length;
	          for (int r = 0; r < unaries.length; r++) {
	            ...
	            double ruleScore = 0;
	            for (int cp = 0; cp < nChildStates; cp++) {
	              double cIS = iScorePostU[start][end][cState][cp];
	              //if (cIS == 0) continue;
