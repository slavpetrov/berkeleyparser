 ...
 */
public class TFloatMap<T> extends AbstractTMap<T>
    implements Iterable<TFloatMap<T>.Entry>, Serializable
{
  ...
  public TFloatMap(Functionality<T> keyFunc) {
  ...
  public TFloatMap(AbstractTMap<T> map) {
    ...
    if(map instanceof TFloatMap)
      this.values = ((TFloatMap<T>)map).values.clone();
    else
   ...
   */
  public TFloatMap(Functionality<T> keyFunc, int expectedSize) {
  ...
  public TFloatMap<T> copy() {
    TFloatMap<T> newMap = new TFloatMap<T>(keyFunc);
  ...
  private class ValueIterator extends MapIterator<Float>
	{
		public Float next()
		{
...
public String toString(){
	  ...
	  sb.append("[");
	  for (TFloatMap<T>.Entry entry : entrySet()){
