...
public class StateSetTreeList extends AbstractCollection<Tree<StateSet>> {
  ...
  /**
	...
	private Tree<StateSet> copyTree(Tree<StateSet> tree) {
  	ArrayList<Tree<StateSet>> newChildren = new ArrayList<Tree<StateSet>>(tree.getChildren().size());
  	for (Tree<StateSet> child : tree.getChildren()) {
  ...
  public static Tree<StateSet> stringTreeToStatesetTree (Tree<String> tree, short[] numStates, boolean allSplitTheSame, Numberer tagNumberer){
    Tree<StateSet> result = stringTreeToStatesetTree(tree,numStates,allSplitTheSame,tagNumberer,false,0,tree.getYield().size());
...
//      return new Tree<StateSet>(newState);
    ...
    for (Tree<String> child : tree.getChildren()) {
      ...
      Tree<StateSet> newChild = stringTreeToStatesetTree(child, numStates, allSplitTheSame, tagNumberer, true, from, from+length);
      from += length;
  ...
  private static Tree<StateSet> resizeStateSetTree (Tree<StateSet> tree, short[] numStates, boolean constant) {
    ...
    nSub[1] = 1;
    for (Tree<String> tree : trees) {
      Tree<StateSet> tmp = stringTreeToStatesetTree(tree, nSub, true, tagNumberer);
    }
	}
}
