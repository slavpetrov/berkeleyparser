...
//  	}
  ...
  @SuppressWarnings("unchecked")
	public SophisticatedLexicon(short[] numSubStates, int smoothingCutoff, double[] smoothParam, 
    ...
    if (!(Corpus.myTreebank == Corpus.TreeBankType.WSJ || Corpus.myTreebank == Corpus.TreeBankType.BROWN
    		|| Corpus.myTreebank == Corpus.TreeBankType.SINGLEFILE))
    	unknownLevel = 4;
  }
...
//			}
		...
		SophisticatedLexicon lexicon = new SophisticatedLexicon(newNumSubStates, this.smoothingCutoff, smooth, smoother, this.threshold);
...
//	  	 just add the signature with count 0 so that we have seen it
  ...
  protected String getCachedSignature(String word, int sentencePosition) {
  	if (word==null) return lastWordToSignaturize;
...
//		else if(isKnown(word))return getConditionalSignatureScore(word, tag, noSmoothing);
	...
	/* (non-Javadoc)
	 * @see edu.berkeley.nlp.PCFGLA.Lexicon#getLinearIndex(java.lang.String, int)
	 */
	...
	/* (non-Javadoc)
	 * @see edu.berkeley.nlp.PCFGLA.Lexicon#scoreSignature(java.lang.String, int, int)
	 */
	...
	public SophisticatedLexicon remapStates(Numberer thisNumberer, Numberer newNumberer) {
		...
		remappedLexicon.smoother = smoother.remapStates(thisNumberer, newNumberer);
		if (conditionalWeights != null) {
		...
		for (short s=0; s<newNumberer.size(); s++) {
			...
			if (translatedState >= 0) {
				...
				if (allTags.contains(translatedState)) remappedLexicon.allTags.add(s);
