...
public final class SloppyMath {
	...
	public static double lambert(double v, double u) {
		...
		/*
		 * //Use asymptotic expansion w = log(z) - log(log(z)) for most z double
		 * summand = (z==0) ? 1 : 0; double tmp = Math.log(z+summand);// +
		 * i*b*6.28318530717958648; double w = tmp - Math.log(tmp + summand);
		 * 
		 * //For b = 0, use a series expansion when close to the branch point
		 * //k = find(b == 0 & abs(z + 0.3678794411714423216) <= 1.5); tmp =
		 * Math.sqrt(5.43656365691809047*z + 2) - 1;// +
		 * i*b*6.28318530717958648; //w(k) = tmp(k); w = tmp;
		 * 
		 * for (int k=1; k<36; k++){ // Converge with Halley's iterations, about
		 * 5 iterations satisfies //the tolerance for most z double c1 =
		 * Math.exp(w); double c2 = w*c1 - z; summand = (w != -1) ? 1 : 0;
		 * double w1 = w + summand; double dw = c2/(c1*w1 - ((w +
		 * 2)*c2/(2*w1))); w = w - dw;
		 * 
		 * if (Math.abs(dw) < 0.7e-16*(2+Math.abs(w))) break; } return w;
		 */
	}
	 ...
	 * same value, the result is that same value. Does none of the special
	 ...
	 * value, the result is that same value. Does none of the special checks for
	 * NaN or -0.0f that <code>Math.max</code> does.
	 * 
	 * @param a
	 *            an argument.
	 * @param b
	...
	public static double oneTailedFishersExact(int k, int n, int r, int m) {
		if (k < 0 || k < (m + r) - n || k > r || k > m || r > n || m > n) {
			throw new IllegalArgumentException("Invalid Fisher's exact: "
					+ "k=" + k + " n=" + n + " r=" + r + " m=" + m + " k<0="
					+ (k < 0) + " k<(m+r)-n=" + (k < (m + r) - n) + " k>r="
					+ (k > r) + " k>m=" + (k > m) + " r>n=" + (r > n) + "m>n="
					+ (m > n));
		}
		// exploit symmetry of problem
		if (m > n / 2) {
	...
	public static void main(String[] args) {
		...
		// if (args.length == 0) {
		// System.err.println("Usage: java edu.stanford.nlp.math.SloppyMath " +
		// "[-logAdd|-fishers k n r m|-bionomial r n p");
		// } else if (args[0].equals("-logAdd")) {
		// System.out.println("Log adds of neg infinity numbers, etc.");
		// System.out.println("(logs) -Inf + -Inf = " +
		// logAdd(Double.NEGATIVE_INFINITY, Double.NEGATIVE_INFINITY));
		// System.out.println("(logs) -Inf + -7 = " +
		// logAdd(Double.NEGATIVE_INFINITY, -7.0));
		// System.out.println("(logs) -7 + -Inf = " + logAdd(-7.0,
		...
		// System.out.println("real 1/2 + 1/2 = " + logAdd(Math.log(0.5),
		// Math.log(0.5)));
		...
		// System.out.println("hypg(" + cell12 + "; " + n + ", " + othcol + ", "
		// + m + ") = " + ans);
		// ans = SloppyMath.hypergeometric(cell21, n, r, othrow);
		// System.out.println("hypg(" + cell21 + "; " + n + ", " + r + ", " +
		// othrow + ") = " + ans);
		// ans = SloppyMath.hypergeometric(cell22, n, othcol, othrow);
		// System.out.println("hypg(" + cell22 + "; " + n + ", " + othcol + ", "
		// + othrow + ") = " + ans);
		...
		// System.out.println("Binomial p(X >= " + k + "; " + n + ", " + p +
		// ") = " + ans);
		// }
