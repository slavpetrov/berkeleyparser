...
public class ConstrainedMaxRuleParser extends ConstrainedSumParser {
  ...
  public ConstrainedMaxRuleParser(Grammar gr, LexiconInterface lex, short[] nSub, double unaryPenalty) {
    super(gr, lex, nSub);
    System.out.println("The unary penalty for parsing is "+unaryPenalty+".");
    this.unaryPenalty = unaryPenalty;
    sumOutPOS = false;//true;
      ...
      for (int start = 0; start < (length - diff + 1); start++) {
         ...
         if (diff > 1) {
          ...
          for (int pState : possibleSt) {
            ...
            for (int i = 0; i < parentRules.length; i++) {
              ...
              boolean hasPOSchild = false;
              if (sumOutPOS) {
              	hasPOSchild = !(grammar.isGrammarTag(lState)&&grammar.isGrammarTag(rState));
              }
              ...
              for (int split = min; split <= max; split++) {
								...
								if (sumOutPOS && hasPOSchild){
									maxcScore[start][end][pState] += gScore;
									if (gScore > maxsScore[start][end][pState]) {
								    maxsScore[start][end][pState] = gScore;
								    maxcSplit[start][end][pState] = split;
								    maxcLeftChild[start][end][pState] = lState;
								    maxcRightChild[start][end][pState] = rState;
								    if (split-start==1 && !grammar.isGrammarTag(lState)) maxcLeftChild[start][end][pState] = posTAG;
								    if (end-split==1 && !grammar.isGrammarTag(rState)) maxcRightChild[start][end][pState] = posTAG;
									}
								}
								else {                	
								  if (gScore > maxcScore[start][end][pState]) {
								    maxcScore[start][end][pState] = gScore;
								    if (sumOutPOS) maxsScore[start][end][pState] = gScore;
								    maxcSplit[start][end][pState] = split;
								    maxcLeftChild[start][end][pState] = lState;
								    maxcRightChild[start][end][pState] = rState;
								  }
        ...
        for (int pState : possibleSt){
          ...
          for (int r = 0; r < unaries.length; r++) {
            ...
            boolean hasPOSchild = false;
            if (sumOutPOS) hasPOSchild = (end-start)==1;//grammar.isGrammarTag(cState);
            ...
            if (sumOutPOS && hasPOSchild){
              ...
              maxcChild[start][end][pState] = posTAG;
            }
            else {
  ...
  public Tree<String>[] getSampledTrees(List<String> sentence, List<Integer>[][] pStates, int n){
    ...
    Tree<String>[] sampledTrees = new Tree[n];
  ...
  public Tree<String> getBestConstrainedParse(List<String> sentence, List<Integer>[][] pStates) {
    ...
    if (iScore[0][length][0]==null) return bestTree;
    double score = iScore[0][length][0][0];
    if (score > 0) {
