...
/**
 * Essentially equivalent to a List<Tree<StateSet>>, but each Tree<StateSet> is re-built every time
 * from the corresponding Tree<String>. This saves a lot of memory at the expense of some time.
...
public class StateSetTreeList extends AbstractCollection<Tree<StateSet>> {
  ...
  /**
	 ...
	 * @param dontSplitTags
	 */
  public StateSetTreeList(List<Tree<String>> trees, short[] numStates, boolean allSplitTheSame, Numberer tagNumberer, boolean dontSplitTags) {
    this.trees = new ArrayList<Tree<StateSet>>();
    for (Tree<String> tree : trees) {
      this.trees.add(stringTreeToStatesetTree(tree, numStates, allSplitTheSame, tagNumberer, dontSplitTags));
    }
  }
   ...
   * @return
   */
  public static Tree<StateSet> stringTreeToStatesetTree (Tree<String> tree, short[] numStates, boolean allSplitTheSame, Numberer tagNumberer, boolean dontSplitTags){
    Tree<StateSet> result = stringTreeToStatesetTree(tree,numStates,allSplitTheSame,tagNumberer,false,dontSplitTags,0,tree.getYield().size());
  ...
  private static Tree<StateSet> stringTreeToStatesetTree (Tree<String> tree, short[] numStates, boolean allSplitTheSame, Numberer tagNumberer, boolean splitRoot, boolean dontSplitPOS, int from, int to){
    if (tree.isLeaf()) {
    ...
    if (!splitRoot || (dontSplitPOS && tree.isPreTerminal())) nodeNumStates = 1;
    StateSet newState = new StateSet(label, nodeNumStates, "", (short)from , (short)to);
    ...
    for (Tree<String> child : tree.getChildren()) {
      ...
      Tree<StateSet> newChild = stringTreeToStatesetTree(child, numStates, allSplitTheSame, tagNumberer, true, dontSplitPOS, from, from+length);
      from += length;
