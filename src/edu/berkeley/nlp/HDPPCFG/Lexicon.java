 ...
 */
public class Lexicon implements java.io.Serializable, LexiconInterface {
...
//  	wordToTagCounters = probCounter;
  ...
  public void optimize(DiscreteDistribCollectionFactory ddcFactory, TopLevelWordDistrib[] topDistribs) {
  	// the input arguments are not used!
    ...
    int sentenceScale = trainTree.getLabel().getIScale();
      ...
      for (short substate=0; substate<nSubStates; substate++) {
        ...
        if (randomness == -1) {
          ...
          weight = Math.random()*randomness/100.0+1.0;
        }
        if (weight==0)
        	continue;
...
//		else if(isKnown(word))return getConditionalSignatureScore(word, tag, noSmoothing);
	...
	/* (non-Javadoc)
	 * @see edu.berkeley.nlp.HDPPCFG.LexiconInterface#getSmoother()
	 */
	...
	/* (non-Javadoc)
	 * @see edu.berkeley.nlp.HDPPCFG.LexiconInterface#setSmoother(edu.berkeley.nlp.HDPPCFG.smoothing.Smoother)
	 */
	...
	/* (non-Javadoc)
	 * @see edu.berkeley.nlp.HDPPCFG.LexiconInterface#setUseVarDP(boolean)
	 */
